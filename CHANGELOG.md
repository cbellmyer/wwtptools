...
- **Media optimization pipeline** — Infrastructure for images, diagrams, and motion in the guides and matrix, built on Hugo Extended's native image processing so no external build framework is required:
  - _Shared engine_ — `layouts/partials/responsive-image.html` renders raster sources (PNG/JPEG) as a `<picture>` with a WebP source plus an original-format fallback, both width-described `srcset` at 480/800/1200/1600px (never upscaled past the source), `loading="lazy"`, `decoding="async"`, and intrinsic `width`/`height` + `aspect-ratio` to prevent layout shift (CLS). SVG, GIF, and remote sources pass through unchanged.
  - _Authoring paths_ — Plain markdown `![alt](src)` is optimized automatically via a project-level `render-image` hook (overrides the theme passthrough). The `{{< img >}}` shortcode was upgraded to WebP/`<picture>` and made crash-safe on SVG; `{{< figure >}}` is overridden to route through the engine while keeping the theme's caption/link API.
  - _GIF replacement_ — `scripts/convert-images.sh` (ffmpeg) transcodes animated GIFs to MP4 + WebM with a poster frame; the new `{{< video >}}` shortcode embeds them autoplay/muted/loop/playsinline. A multi-MB GIF typically becomes a ~100–300 KB clip.
  - _Stock stand-ins_ — `content/assets/images/stock/` ships `placeholder-diagram.svg` and `placeholder-photo.png` as replaceable defaults for forks.
  - _Authoring guide_ — `docs/IMAGES.md` documents the workflow and rationale.
...
- **Process Decision Matrix — Activated Sludge draft** — First working zone of the Process Decision Matrix, built as a guided, symptom-driven troubleshooter. Navigation is staged so an operator is never faced with a wall of options: pick a treatment area (Aeration Basin or Secondary Clarifier), choose the symptom being observed, then work a branching question tree (`q1 → q2 → …`) that resolves to a diagnosis with probable causes, numbered corrective actions, and an operator note. The Activated Sludge zone covers 16 symptoms split across the two subsystems (9 aeration, 7 clarifier), backed by ~50 diagnosis records; a back control returns to the area list and a path breadcrumb tracks the questions answered. A filter box searches symptoms across both areas and jumps straight past the area step. Each symptom shows a severity dot (low / moderate / high) derived from the worst diagnosis reachable on its tree. All content is data-driven in `data/matrix.yaml` — a `subsystems` list defines the areas, each symptom is tagged with a `group`, and adding or editing symptoms, questions, or diagnoses requires only editing the data file. Rendered by a vanilla-JS click-through UI (no framework). Status bar set to "Draft"; zone cards mark Activated Sludge as live; the weather widget sits directly below the draft status bar, above the active tool.
- **Process Decision Matrix — inline figures** — Schematic SVG figures embedded in the diagnostic flow, drawn to match the site's technical aesthetic (line diagrams with restrained accent color). The four appearance-based questions — foam type, settleometer settling, mixed-liquor color, and effluent solids — present their answer choices as labeled thumbnails so an operator can match by sight instead of parsing text. Four geometry-driven diagnoses carry a captioned diagram: a settleometer at 30 min for filamentous bulking, a clarifier cross-section with a rising blanket for the two blanket-rise paths, and a diffuser-grid schematic with a broken lateral for aeration-equipment DO loss. Figures are referenced by key from the data (`figure:` / `figureCaption:` on options and diagnoses) and defined once in the matrix template, so coverage extends by editing the data file. Every figure is inlined as well-formed SVG, adding no network requests.

- **Interface modernization — card-style redesign** — Sitewide visual refresh moving from the boxed SCADA-panel look toward a modern elevated-card interface, while keeping the dark slate palette, sky-blue accent system, and monospace data typography. A design-token layer added to `wwtptools.css` (`--radius-sm/md/lg/pill` scale, three `--shadow-*` elevation levels, soft border tints `--border-soft`/`--border-faint`, and `--ease-out`/`--dur-*` motion variables) drives all component styling. Calculator cards, pie wheels, guide cards, module cards, matrix zone cards, reference tables, and the weather panel are now borderless rounded surfaces (14px radius) with layered soft shadows and real grid gutters, replacing the 1px-gap bordered cell grids. Link cards lift 2–3px on hover; calculator cards deepen elevation on hover/focus-within without moving, so inputs stay put under the cursor. Buttons are pills with hover lift (primary gets a blue glow shadow); calculator inputs gained rounded corners and a sky-blue focus ring (previously amber). In the decision matrix, question and diagnosis cards animate in with a fade-up, answer options are rounded buttons that lift on hover, symptom rows indent on hover, the dashed "awaiting input" box became a soft tinted panel, and severity badges, area counts, and quick-jump nav links are pills. All transitions run 150–250ms on an ease-out curve; a global `prefers-reduced-motion` media query collapses animation and transitions for users who opt out.
- **Matrix page — weather widget repositioned** — Moved from below the zone cards to directly beneath the [ Draft ] status bar, so current conditions and active weather alerts are visible before the diagnostic tool rather than after it.

### Fixed

- **Readability — minimum label font size** — Monospace micro-labels across the toolkit, matrix, and site chrome had drifted as low as 0.38rem (~7px rendered), well below readable size. All label font sizes are now floored at 0.6–0.65rem (~11px) in both stylesheets, mapped in tiers so the existing size hierarchy is preserved.
